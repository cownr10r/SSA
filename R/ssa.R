...
#' @param d the last number of the row indicating the last minute of observation
#' @param e a number indicating the minimum number for transitions between sequences
#' @import docxtractr
#' @import TraMineR
#' @import dplyr
#' @import gridExtra
...
ssa <- function(a,b,d,e = 1){
  ...
  mvad.seq <- TraMineR::seqdef(data, b:d, alphabet = st.alphabet, states = st.scodes, labels = st.labels)
 ...
 fsubseq <- TraMineR::seqefsub(mvad.seqe, min.support = e)
